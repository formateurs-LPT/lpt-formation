      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', animation: 'pdmHaloPulse 1.5s ease-in-out infinite' }} />
            <span style={{ fontSize: 10, fontWeight: 700, color: '#f59e0b', letterSpacing: .5 }}>EN DIRECT</span>
          </div>
        </div>
      </div>
    </div>
...
export default function ModulePDM({ pName, onBack }) {
  ...
  const handleEndQuiz = async () => {
    await sbUpdate('sessions', { active_module: 'pdm', module_page: 200 }, 'code=eq.' + SESSION_CODE)
  ...
  useEffect(() => {
    if (started) {
      sbUpdate('sessions', { active_module: 'pdm', module_page: 0 }, 'code=eq.' + SESSION_CODE)
    }
  }, [started])
  ...
  useEffect(() => {
    if (started) {
      sbUpdate('sessions', { module_page: pageIndex }, 'code=eq.' + SESSION_CODE)
    }
  }, [pageIndex, started])
