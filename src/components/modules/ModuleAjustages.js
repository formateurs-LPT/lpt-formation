    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', display: 'flex', flexDirection: 'column', padding: '20px 32px 32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', fontWeight: 500 }}>SAV · Ajustages — Brainstorm</span>
        </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <div style={{ display: 'inline-block', background: 'rgba(167,139,250,0.15)', border: '1px solid rgba(167,139,250,0.35)', borderRadius: 20, padding: '5px 20px', fontSize: 11, fontWeight: 700, color: '#a78bfa', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 16 }}>💬 Brainstorm — Les formés répondent sur leur téléphone</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20 }}>
        ...
        <button onClick={handleNext} style={{ background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', border: 'none', color: '#fff', padding: '13px 32px', borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 5px 20px rgba(124,58,237,0.4)' }}>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 28px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Image src="/assets/logo-lpt-blanc.png" alt="LPT" width={80} height={30} style={{ objectFit: 'contain' }} />
          <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.15)' }} />
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', fontWeight: 500 }}>SAV · Ajustages · {pageIndex + 1}/{total}</span>
        </div>
        <button onClick={onBack}
          style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.55)', padding: '7px 16px', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,80,80,0.18)'; e.currentTarget.style.color = '#ff6b6b' }}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <div style={{ flex: '3', padding: '24px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'center', borderRight: '1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ opacity: entered ? 1 : 0, transform: entered ? 'translateX(0)' : 'translateX(-24px)', transition: 'all 0.5s ease' }}>
      ...
      <div style={{ padding: '12px 28px 16px', flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={onPrev} disabled={isFirst} style={{ background: isFirst ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.15)', color: isFirst ? 'rgba(255,255,255,0.2)' : '#fff', padding: '11px 22px', borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: isFirst ? 'default' : 'pointer', fontFamily: 'inherit' }}>← Précédent</button>
          {isLast ? (
            quizLaunched ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ color: '#4ade80', fontWeight: 700, fontSize: 13 }}>Quiz envoyé</span>
                <button onClick={onTerminate} style={{ background: 'linear-gradient(135deg, #16a34a, #22c55e)', border: 'none', color: '#fff', padding: '11px 28px', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Terminer</button>
              </div>
            ) : (
              <button onClick={onLaunchQuiz} style={{ background: 'linear-gradient(135deg, #7c3aed, #9f67fa)', border: 'none', color: '#fff', padding: '11px 28px', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 6px 24px rgba(124,58,237,0.5)' }}>🧠 Lancer le quiz →</button>
            )
          ) : (
            <button onClick={onNext} style={{ background: `linear-gradient(135deg, ${color}, ${color}cc)`, border: 'none', color: '#fff', padding: '11px 28px', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: `0 5px 20px ${color}40` }}>Suivant →</button>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      <div style={{ textAlign: 'center', maxWidth: 560, padding: '0 24px' }}>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: '#a78bfa', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 10 }}>SAV · Journée 4 · Module 2</div>
        <h1 style={{ fontSize: 32, fontWeight: 800, color: '#fff', marginBottom: 10 }}>Les Ajustages</h1>
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', marginBottom: 36, lineHeight: 1.6 }}>Confort quotidien · Résolution visuelle · Acétate & Métal</p>
        <button onClick={onStart} style={{ background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', border: 'none', color: '#fff', padding: '16px 48px', borderRadius: 16, fontSize: 17, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 32px rgba(124,58,237,0.45)', fontFamily: 'inherit' }}>▶ Démarrer</button>
      </div>
    </div>
  )
}
...
function QuizController({ quizQ, onNext, onEnd, onBack }) {
  ...
  const q = AJUSTAGES_QUIZ[quizQ]
  const isLast = quizQ >= AJUSTAGES_QUIZ.length - 1
  const pageId = `ajustages:${quizQ}`

  useEffect(() => {
    setOpenAnswers([]); setValidating({}); setValidated({})
    const poll = async () => { const rows = await fetchOpenAnswers(getActiveSessionCode(), pageId); setOpenAnswers(rows || []) }
    poll()
    const t = setInterval(poll, 2000)
    return () => clearInterval(t)
  }, [quizQ, pageId])
  ...
  const handleValidate = async (row, isCorrect) => {
    ...
    await saveModuleQuizAnswer({ moduleId: 'ajustages', questionIdx: quizQ, collaborateur: row.participant_name, answerIdx: 0, isCorrect })
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', padding: '24px clamp(14px, 4vw, 48px) 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Image src="/assets/logo-lpt-blanc.png" alt="LPT" width={80} height={30} style={{ objectFit: 'contain' }} />
          <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.15)' }} />
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Quiz · Ajustages — Vue formateur</span>
        </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <div style={{ display: 'inline-block', background: 'rgba(167,139,250,0.2)', border: '1px solid rgba(167,139,250,0.4)', borderRadius: 20, padding: '6px 24px', fontSize: 12, fontWeight: 700, color: '#a78bfa', letterSpacing: 1.5, textTransform: 'uppercase' }}>
          Question {quizQ + 1} / {AJUSTAGES_QUIZ.length}
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <div style={{ display: 'inline-block', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 12, padding: '8px 22px', fontSize: 12, color: 'rgba(167,139,250,0.8)', fontStyle: 'italic' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 820, margin: '0 auto', maxHeight: '45vh', overflowY: 'auto' }}>
        {openAnswers.length === 0 ? (
          ...
          const isV = validating[row.participant_name]
          return (
            <div key={row.participant_name} style={{ background: status === 'correct' ? 'rgba(34,197,94,0.08)' : status === 'wrong' ? 'rgba(239,68,68,0.08)' : 'rgba(255,255,255,0.04)', border: `1px solid ${status === 'correct' ? 'rgba(34,197,94,0.3)' : status === 'wrong' ? 'rgba(239,68,68,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: `${BUBBLE_COLORS[i % BUBBLE_COLORS.length]}22`, border: `2px solid ${BUBBLE_COLORS[i % BUBBLE_COLORS.length]}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: BUBBLE_COLORS[i % BUBBLE_COLORS.length] }}>{row.participant_name?.charAt(0)?.toUpperCase() || '?'}</div>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                {status ? (
                  ...
                  <>
                    <button onClick={() => handleValidate(row, true)} disabled={!!isV} style={{ width: 38, height: 38, borderRadius: 10, border: 'none', background: 'rgba(34,197,94,0.18)', color: '#4ade80', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: isV ? 0.5 : 1, fontFamily: 'inherit' }}>✓</button>
                    <button onClick={() => handleValidate(row, false)} disabled={!!isV} style={{ width: 38, height: 38, borderRadius: 10, border: 'none', background: 'rgba(239,68,68,0.18)', color: '#f87171', fontSize: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: isV ? 0.5 : 1, fontFamily: 'inherit' }}>✗</button>
                  </>
                )}
              </div>
            </div>
      ...
      {openAnswers.length > 0 && <div style={{ textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.3)', marginTop: 10 }}>{openAnswers.length} réponse{openAnswers.length > 1 ? 's' : ''} reçue{openAnswers.length > 1 ? 's' : ''}</div>}
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 28 }}>
        {isLast ? (
          ...
          <button onClick={onNext} style={{ background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', border: 'none', color: '#fff', padding: '14px 40px', borderRadius: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 6px 24px rgba(124,58,237,0.45)' }}>Question suivante →</button>
        )}
      </div>
    </div>
  )
...
function GroupResultsView({ onTerminate }) {
  ...
  useEffect(() => {
    fetchTrainerQuizAnswers(`session_code=eq.${getActiveSessionCode()}&module_id=eq.ajustages`).then(rows => { setAnswers(rows || []); setLoading(false) })
  }, [])
  const participantCount = [...new Set((answers || []).map(r => r.collaborateur))].length
  const questionStats = AJUSTAGES_QUIZ.map((q, idx) => {
    const qA = answers.filter(r => r.question_idx === idx)
    const wrong = qA.filter(r => !r.is_correct).length
    const pct = qA.length > 0 ? Math.round((wrong / qA.length) * 100) : 0
    return { idx, question: q.question, pctWrong: pct }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', display: 'flex', flexDirection: 'column', padding: '24px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Image src="/assets/logo-lpt-blanc.png" alt="LPT" width={80} height={30} style={{ objectFit: 'contain' }} />
          <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.15)' }} />
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>Bilan du quiz · Ajustages</span>
        </div>
        <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)' }}><span style={{ fontSize: 22, fontWeight: 800, color: '#fff', marginRight: 6 }}>{participantCount}</span>participant{participantCount !== 1 ? 's' : ''}</div>
      </div>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <div style={{ display: 'inline-block', background: 'rgba(167,139,250,0.15)', border: '1px solid rgba(167,139,250,0.35)', borderRadius: 20, padding: '6px 24px', fontSize: 12, fontWeight: 700, color: '#a78bfa', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>Résultats du groupe</div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 760, alignSelf: 'center', width: '100%' }}>
        {loading ? <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.4)', padding: 40 }}>Chargement…</div>
              ...
              <div key={stat.idx} style={{ background: p.bg, border: `1px solid ${p.border}`, borderRadius: 18, padding: '18px 22px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  <div style={{ flex: 1, marginRight: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><span style={{ fontSize: 16 }}>{p.icon}</span><span style={{ fontSize: 11, fontWeight: 700, color: p.color, textTransform: 'uppercase', letterSpacing: 1 }}>{p.label}</span></div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: '#fff', lineHeight: 1.4 }}>Q{stat.idx + 1} — {stat.question}</div>
                  </div>
                  <div style={{ fontSize: 40, fontWeight: 800, color: p.color, lineHeight: 1, flexShrink: 0 }}>{stat.pctWrong}%<div style={{ fontSize: 11, fontWeight: 500, color: 'rgba(255,255,255,0.4)', textAlign: 'right' }}>d'erreurs</div></div>
                </div>
...
export default function ModuleAjustages({ pName, onBack, onTerminate }) {
  ...
  const handleStart = async () => { await sbUpdate('sessions', { active_module: 'ajustages', module_page: 0 }, 'code=eq.' + sc()); setStarted(true) }
  ...
  const handleLaunchQuiz = async () => { await sbUpdate('sessions', { active_module: 'ajustages', module_page: 100 }, 'code=eq.' + sc()); setQuizQ(0); setQuizLaunched(true) }
  const handleNextQuestion = async () => { const n = quizQ + 1; await sbUpdate('sessions', { active_module: 'ajustages', module_page: 100 + n }, 'code=eq.' + sc()); setQuizQ(n) }
  const handleEndQuiz = async () => { await sbUpdate('sessions', { active_module: 'ajustages', module_page: 200 }, 'code=eq.' + sc()); setShowGroupResults(true) }
  ...
  const currentPage = AJUSTAGES_PAGES[page]
  ...
  const contentIndex = page - 1
  const nextPage = page < TOTAL_PAGES - 1 ? { type: AJUSTAGES_PAGES[page + 1].type, color: AJUSTAGES_PAGES[page + 1].color, label: AJUSTAGES_PAGES[page + 1].titre } : null

  return (
    <SAVPage
      page={currentPage}
      onBack={handleBack} onPrev={handlePrev} onNext={handleNext}
      isFirst={false} isLast={page >= TOTAL_PAGES - 1}
      pageIndex={contentIndex} total={contentTotal}
      nextPage={nextPage} onTerminate={handleTerminate}
      quizLaunched={quizLaunched} onLaunchQuiz={handleLaunchQuiz}
    />
  )
}
