...
function QuizController({ quizQ, onNext, onEnd, onBack }) {
  ...
  const handleValidate = async (row, isCorrect) => {
    ...
    try {
      await saveModuleQuizAnswer({ sessionCode: getActiveSessionCode(), moduleId: 'ajustages', questionIdx: quizQ, collaborateur: row.participant_name, answerIdx: 0, isCorrect })
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', padding: '24px clamp(14px, 4vw, 48px) 40px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 28 }}>
        ...
        {isLast ? (
          ...
          <button onClick={() => { if (confirmSkipPending()) onNext() }} style={{ background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', border: 'none', color: '#fff', padding: '14px 40px', borderRadius: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 6px 24px rgba(124,58,237,0.45)' }}>Question suivante →</button>
        )}
      </div>
    </div>
  )
...
export default function ModuleAjustages({ pName, onBack, onTerminate }) {
  ...
  const handleNextQuestion = async () => {
    const n = quizQ + 1
    await setSharedState({ quiz_show_correction: false }).catch(() => {})
    await sbUpdate('sessions', { active_module: 'ajustages', module_page: 100 + n }, 'code=eq.' + sc())
    setQuizQ(n)
  }
  const handleEndQuiz = async () => {
    try { await sbUpdate('sessions', { active_module: 'ajustages', module_page: 200 }, 'code=eq.' + sc()) } catch { /* best-effort */ }
