...
function QuizController({ quizQ, onNext, onEnd, onBack }) {
  ...
  useEffect(() => {
    const poll = async () => {
      const rows = await sbSelect('quiz_answers', `session_code=eq.${SESSION_CODE}&module_id=eq.types-verres&question_idx=eq.${quizQ}`)
      setLiveAnswers(rows || [])
    }
    poll()
...
function GroupResultsView({ onTerminate }) {
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      const rows = await sbSelect('quiz_answers', `session_code=eq.${SESSION_CODE}&module_id=eq.types-verres`)
      setAnswers(rows || [])
      setLoading(false)
    }
    fetchAnswers()
...
export default function ModuleTypesVerres({ pName, onBack }) {
  ...
  useEffect(() => {
    if (started) {
      sbUpdate('sessions', { active_module: 'types-verres', module_page: 0 }, 'code=eq.' + SESSION_CODE)
    }
  }, [started])
