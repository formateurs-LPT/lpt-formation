  ...
  return (
    <div style={{
      ...
      <h1 style={{ fontSize: 30, fontWeight: 800, color: '#fff', marginBottom: 8 }}>
        Les offres et la vente
      </h1>
      <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', marginBottom: 36, lineHeight: 1.6 }}>
        Trame d&apos;accueil · Offre 1=1 · Offre Classique · Pack Plan · Ordonnances<br />
        {QUIZ_J2.length} questions · Classements toutes les 5 questions
      </p>
...
// ── Aperçu trame d'accueil (formateur) — question 1 uniquement ──────
function TramePreview({ q }) {
  if (!q.tramePoints) return null
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 640, margin: '0 auto 16px' }}>
      {q.tramePoints.map(pt => (
        <div key={pt.num} style={{
          display: 'flex', alignItems: 'center', gap: 12,
          background: pt.text ? `${pt.color}0c` : 'rgba(255,255,255,0.03)',
          border: `1px solid ${pt.text ? pt.color + '35' : 'rgba(255,255,255,0.15)'}`,
          borderLeft: `3px solid ${pt.color}`,
          borderRadius: 10, padding: '8px 14px',
        }}>
          <span style={{ fontSize: 16, flexShrink: 0 }}>{pt.emoji}</span>
          <span style={{ fontSize: 13, color: pt.text ? '#fff' : 'rgba(255,255,255,0.35)', fontStyle: pt.text ? 'normal' : 'italic' }}>
            {pt.text || `Case ${pt.num} — à compléter par les formés`}
          </span>
        </div>
      ))}
    </div>
  ...
  useEffect(() => {
    ...
    const autoValidate = (rows) => {
      ...
      for (const row of rows) {
        ...
        if (matchOr || matchAnd || matchAllOr || matchEach) {
        ...
        {answers.length === 0 ? (
          ...
          return (
            <div key={row.participant_name} style={{
              ...
              borderRadius: 14, padding: '14px 18px', animation: 'j2FadeIn .3s ease',
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                ...
                {status === undefined ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                    <span style={{ fontSize: 22, color: status ? '#4ade80' : '#f87171' }}>{status ? '✓' : '✗'}</span>
                    {/* Correction manuelle possible même après une validation (auto ou manuelle) — utile pour les questions auto-corrigées (Q6/Q7) en cas de litige */}
                    <button
                      onClick={() => handleValidate(row.participant_name, !status)}
                      title="Corriger cette validation"
                      style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.45)', padding: '5px 10px', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                    >↺ Changer</button>
                  </div>
                )}
              </div>
...
// ── Contrôleur questions standard (QCM, ordonnance) ──────────────────
...
function GroupResultsView({ onTerminate }) {
  ...
  const questionStats = QUIZ_J2.map((q, idx) => {
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #03112a 0%, #0a2a5c 55%, #0d3b7a 100%)', display: 'flex', flexDirection: 'column', padding: '24px 28px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Bilan — Quiz Jour 2</span>
        </div>
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
...
export default function ModuleQuizJ2({ pName, onBack }) {
  ...
  const q = QUIZ_J2[quizQ]
  const type = q?.type || 'qcm'
  const isLast = quizQ >= QUIZ_J2.length - 1

  return (
    <>
      <style>{STYLES}</style>
      {(type === 'text-open' || type === 'text-open-multi') ? (
