...
export default function AutoEvalView({ onBack }) {
  ...
  const launch = async () => {
    setToggling(true)
    await setSharedState({ tv_screen: 'auto-eval' })
    setIsActive(true)
    setToggling(false)
  }
  ...
  return (
    <div id="dashboard" style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden', background: '#f8fafc' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.5 }}>
                  Les formés rempliront un questionnaire sur leur téléphone<br />
                  (thèmes · progrès · appréciation · suggestions)
                </div>
              </div>
              <button
                onClick={launch}
        ...
        {/* Réponses hors-liste (formés sans entrée) */}
