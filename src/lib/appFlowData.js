...
// OnboardingView.js et OnboardingViewBelgique.js — reflète le vrai câblage de
// navigation à la date de rédaction. À retoucher si un module change de
// journée ou si un nouveau module est ajouté.
//
// kind:   'root' | 'branch' | 'module' | 'quiz' | 'tool'
...
export const APP_FLOW_NODES = [
  { id: 'dashboard',           label: 'Dashboard formateur',              kind: 'root',   x: 0,       y: 2 * ROW },
  { id: 'onboarding-france',   label: 'Onboarding France',                kind: 'branch', x: COL,     y: 0,       flag: 'fr' },
  { id: 'onboarding-belgique', label: 'Onboarding Belgique',              kind: 'branch', x: COL,     y: 2 * ROW, flag: 'be' },
  { id: 'modules-libres',      label: 'Modules (bibliothèque)',           kind: 'branch', x: COL,     y: 4 * ROW },
  ...
  { id: 'quiz-j1',       label: 'Quiz Jour 1',                  kind: 'quiz',   journee: 'J1',                 x: 6 * COL, y: 2 * ROW },

  // ── J2 — commun ──────────────────────────────────────────────────
  { id: 'trame-accueil', label: "Trame d'accueil", kind: 'module', journee: 'J2', hasQuiz: false, x: 7 * COL, y: 2 * ROW },
  { id: 'offres',        label: 'Les offres',      kind: 'module', journee: 'J2', hasQuiz: true,  x: 8 * COL, y: 2 * ROW, note: 'Contenu additionnel "LPT Care" en session Belgique' },
  { id: 'quiz-j2',       label: 'Quiz Jour 2',     kind: 'quiz',   journee: 'J2',                 x: 9 * COL, y: 2 * ROW },

  // ── J3 — commence commun (PDM), puis diverge ────────────────────
  { id: 'pdm', label: 'Prise de mesures', kind: 'module', journee: 'J3', hasQuiz: true, x: 10 * COL, y: 2 * ROW },

  { id: 'parcours-rembourses',   label: 'Les parcours remboursés',           kind: 'module', journee: 'J3', hasQuiz: false, flag: 'fr', x: 11 * COL, y: 1 * ROW },
  { id: 'remboursement-france',  label: 'Remboursement optique en France',   kind: 'module', journee: 'J3', hasQuiz: true,  flag: 'fr', x: 12 * COL, y: 1 * ROW },
  { id: 'lpt-sante',             label: 'LPT Santé',                         kind: 'module', journee: 'J3', hasQuiz: false, flag: 'fr', x: 13 * COL, y: 1 * ROW },

  { id: 'mutuelles-inami', label: 'Mutuelles et INAMI', kind: 'module', journee: 'J3', hasQuiz: false, flag: 'be', x: 11 * COL, y: 3 * ROW },

  // ── J4 — reconverge, commun ──────────────────────────────────────
  { id: 'retraits',        label: 'Les Retraits',    kind: 'module', journee: 'J4', hasQuiz: true,  x: 14 * COL, y: 2 * ROW },
  { id: 'ajustages',       label: 'Les Ajustages',   kind: 'module', journee: 'J4', hasQuiz: true,  x: 15 * COL, y: 2 * ROW },
  { id: 'raz',             label: 'Les RAZ',         kind: 'module', journee: 'J4', hasQuiz: false, x: 16 * COL, y: 2 * ROW },
  { id: 'montage',         label: 'Le montage',      kind: 'module', journee: 'J4', hasQuiz: false, x: 17 * COL, y: 2 * ROW },
  { id: 'montures-outlet', label: 'Montures Outlet', kind: 'module', journee: 'J4', hasQuiz: false, x: 18 * COL, y: 2 * ROW },

  // ── Outils de session — pas rattachés à une journée précise ─────
  ...
  // ── Isolé — accessible uniquement via "Modules (bibliothèque)", ──
  // pas rattaché à un parcours Onboarding.
  { id: 'verre-progressif', label: 'Le Verre Progressif', kind: 'module', hasQuiz: true, isolated: true, x: 2 * COL, y: 4 * ROW },
]
...
export const APP_FLOW_EDGES = [
  ...
  { source: 'montures', target: 'quiz-j1' },
  { source: 'quiz-j1', target: 'trame-accueil' },
